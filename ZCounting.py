...
parser.add_argument("-g","--dirMCReweight",help="Directory to root files for pilupe reweighting",default=cmsswbase+"/src/ZCounting/Resources/ReweightHistogram/")
parser.add_argument("-t","--dirMCShape",help="Directory to root file for Z mass template",default=cmsswbase+"/src/ZCounting/Resources/92X_norw_IsoMu27_noIso_MuStaEff_probes.root")
...
for run in data.drop_duplicates('run')['run'].values:
	...
	dateUp_m=ROOT.TDatime(currentYear,int(datestampUp_m[0].split("/")[0]),int(datestampUp_m[0].split("/")[1]),int(datestampUp_m[1].split(":")[0]),int(datestampUp_m[1].split(":")[1]),int(datestampUp_m[1].split(":")[2]))
        ...
        GloeffresB_m=ROOT.calculateDataEfficiency(str(eosFile),args.dirEff,str(run),nMeasurements,goodLSlist[0],goodLSlist[-1],avgpu_m,"Glo",0,2,2,2,2,recLumi_m,mcShapeDir,mcReweightDir)
        GloeffresE_m=ROOT.calculateDataEfficiency(str(eosFile),args.dirEff,str(run),nMeasurements,goodLSlist[0],goodLSlist[-1],avgpu_m,"Glo",1,2,2,2,2,recLumi_m,mcShapeDir,mcReweightDir)
