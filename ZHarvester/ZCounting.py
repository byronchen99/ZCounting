...
parser.add_argument("-p", "--parametrizeType",
                    help="define parametrization: 1 is for extrapolation, 2 is for piece-wise function", default=1)
...
parser.add_argument("-d", "--dirDQM", help="Directory to the input root files from the DQM Offline module",
                    default="/eos/home-d/dwalter/www/ZCounting/DQM-Offline-2018/")
parser.add_argument("-f", "--ByLsCSV", help="ByLs csv input generated by testBril.sh",
                    default="/eos/cms/store/group/comm_luminosity/ZCounting/brilcalcFile2018/*csv")
parser.add_argument("-g", "--dirMCReweight", help="Directory to root files for pilupe reweighting",
                    default=cmsswbase + "/src/ZCounting/Resources/ReweightHistogram/")
parser.add_argument("-t", "--dirMCShape", help="Directory to root file for Z mass template",
...
mcReweightDir = args.dirMCReweight
mcShapeDir = args.dirMCShape
...
maximumLS = 2500
LSperMeasurement = 300  # required number of lumi sections per measurement
...
#  Define Z efficiency correction for pileup - parameters:
if currentYear == 2018:
    ...
    f_ZBBEffCorr = fGen(0.0038, 0.00016, 0., 0., -0., 0., 50, paraType)
    f_ZBEEffCorr = fGen(0.0062, 0.00031, 0., 0., -0., 0., 55, paraType)
    f_ZEEEffCorr = fGen(0.0076, 0.00049, 0., 0., 0., 0., 40, paraType)
    ...
    while len(LSlist) > 0:  # begin next measurement "m"
        ...
        while (recLumi_m < LumiPerMeasurement or len(goodLSlist) < LSperMeasurement or mergeMeasurements) and len(
            ...
            if LSlist[0] > h_yield_Z.GetNbinsX():
                ...
                break
            n0_ls = h_n0.GetBinContent(LSlist[0])
            if n0_ls > 0:
                n0list.append(n0_ls)
                goodLSlist.append(LSlist[0])
                recLumi_m += data_run[data_run['ls'] == LSlist[0]]['recorded(/pb)'].values
            del LSlist[0]
        ...
        Zyieldres_m = ROOT.getZyield(str(eosFile), args.dirEff, "h_mass_yield_Z", str(run), nMeasurements,
                                     goodLSlist[0], goodLSlist[-1], 1, 5, recLumi_m);
        HLTeffresB_m = ROOT.calculateDataEfficiency(str(eosFile), args.dirEff, str(run), nMeasurements, goodLSlist[0],
                                                    goodLSlist[-1], avgpu_m, "HLT", 0, 2, 5, 2, 5, recLumi_m,
                                                    mcShapeDir + "template_HLT.root", mcReweightDir)
        HLTeffresE_m = ROOT.calculateDataEfficiency(str(eosFile), args.dirEff, str(run), nMeasurements, goodLSlist[0],
                                                    goodLSlist[-1], avgpu_m, "HLT", 1, 2, 5, 2, 5, recLumi_m,
                                                    mcShapeDir + "template_HLT.root", mcReweightDir)
        SITeffresB_m = ROOT.calculateDataEfficiency(str(eosFile), args.dirEff, str(run), nMeasurements, goodLSlist[0],
                                                    goodLSlist[-1], avgpu_m, "SIT", 0, 2, 1, 2, 1, recLumi_m,
                                                    mcShapeDir + "template_Sel.root", mcReweightDir)
        SITeffresE_m = ROOT.calculateDataEfficiency(str(eosFile), args.dirEff, str(run), nMeasurements, goodLSlist[0],
                                                    goodLSlist[-1], avgpu_m, "SIT", 1, 2, 1, 2, 1, recLumi_m,
                                                    mcShapeDir + "template_Sel.root", mcReweightDir)
        GloeffresB_m = ROOT.calculateDataEfficiency(str(eosFile), args.dirEff, str(run), nMeasurements, goodLSlist[0],
                                                    goodLSlist[-1], avgpu_m, "Glo", 0, 2, 2, 2, 2, recLumi_m,
                                                    mcShapeDir + "template_Glo.root", mcReweightDir)
        GloeffresE_m = ROOT.calculateDataEfficiency(str(eosFile), args.dirEff, str(run), nMeasurements, goodLSlist[0],
                                                    goodLSlist[-1], avgpu_m, "Glo", 1, 2, 2, 2, 2, recLumi_m,
                                                    mcShapeDir + "template_Glo.root", mcReweightDir)
        #        StaeffresB_m=ROOT.calculateDataEfficiency(str(eosFile),args.dirEff,str(run),nMeasurements,goodLSlist[0],goodLSlist[-1],avgpu_m,"Sta",0,2,2,2,2,recLumi_m,mcShapeDir,mcReweightDir)
        #        StaeffresE_m=ROOT.calculateDataEfficiency(str(eosFile),args.dirEff,str(run),nMeasurements,goodLSlist[0],goodLSlist[-1],avgpu_m,"Sta",1,2,2,2,2,recLumi_m,mcShapeDir,mcReweightDir)
        #        TrkeffresB_m=ROOT.calculateDataEfficiency(str(eosFile),args.dirEff,str(run),nMeasurements,goodLSlist[0],goodLSlist[-1],avgpu_m,"Trk",0,2,2,2,2,recLumi_m,mcShapeDir,mcReweightDir)
        #        TrkeffresE_m=ROOT.calculateDataEfficiency(str(eosFile),args.dirEff,str(run),nMeasurements,goodLSlist[0],goodLSlist[-1],avgpu_m,"Trk",1,2,2,2,2,recLumi_m,mcShapeDir,mcReweightDir)
...
if args.writeSummaryCSV:
    ...
    with open(args.dirCSV + 'Mergedcsvfile.csv', 'w') as file:
        file.write(
            "fill,tdate_end,tdate_begin,ZRate,ZRate_EStatUp,ZRate_EStatDown,instDelLumi,delLumi,delZCount,ZRateUncorrected")
        file.write('\n')
    ...
    with open(args.dirCSV + 'Mergedeffcsvfile.csv', 'wb') as fileTwo:
        fileTwo.write(
            "fill,tdate_begin,tdate_end,ZRate,ZRate_EStatUp,ZRate_EStatDown,instDelLumi,delLumi,delZCount,beginLS,endLS,recLumi,windowarray,Zfpr,Zchi2,"
            "HLTeffB,HLTeffE,SITeffB,SITeffE,GloeffB,GloeffE,"
            # "StaeffB,StaeffE,TrkeffB,TrkeffE,
            "HLTeffB_chi2pass,HLTeffB_chi2fail,HLTeffE_chi2pass,HLTeffE_chi2fail,SITeffB_chi2pass,SITeffB_chi2fail,SITeffE_chi2pass,SITeffE_chi2fail,GloeffB_chi2pass,GloeffB_chi2fail,GloeffE_chi2pass,GloeffE_chi2fail,"
            # "StaeffB_chi2pass,StaeffB_chi2fail,StaeffE_chi2pass,StaeffE_chi2fail,TrkeffB_chi2pass,TrkeffB_chi2fail,TrkeffE_chi2pass,TrkeffE_chi2fail,"
            "ZMCeff,ZMCeffBB,ZMCeffBE,ZMCeffEE,Zeff,ZBBeff,ZBEeff,ZEEeff,pileUp")
        fileTwo.write('\n')
